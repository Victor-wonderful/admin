...
import { KeyRoundIcon, BanIcon, CircleCheckIcon, Loader2Icon, XIcon, CopyIcon, CheckIcon } from "lucide-react";

import { resetMemberPassword, setMemberSuspended } from "@/lib/actions/member-admin";
import { useEscapeKey } from "@/hooks/use-escape-key";
import { cn } from "@/lib/utils";

// 회원 상세 상단 액션 — 비밀번호 재설정(임시 비밀번호 1회 표시) · 계정 정지/해제(사유 필수).
...
export function MemberAdminActions({ memberId, label, suspended, suspendReason, readOnly = false }: { memberId: string; label: string; suspended: boolean; suspendReason: string | null; readOnly?: boolean }) {
