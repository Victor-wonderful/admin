...
// MarketerReferralView 본문 — 페이지(세션 가드)와 분리해 두면 재사용·검증이 쉽다.
export async function MarketerReferralView({ viewerId }: { viewerId: string }) {
  ...
  return (
    <>
      ...
      <div className="flex-1 space-y-4 overflow-auto p-4 lg:p-7">
        <div className="flex flex-col gap-4 rounded-xl bg-feature p-5 text-white shadow-[0_2px_12px_-3px_rgba(16,24,40,0.12)] lg:flex-row lg:items-center lg:justify-between lg:p-6">
          <div className="min-w-0">
            <div className="text-xs font-medium text-white/60">내 초대 코드</div>
            <div className="mt-1 font-mono text-[30px] font-bold tracking-wider lg:text-[34px]">{codeStr}</div>
          </div>
        ...
        <Panel title="초대 전환 퍼널" sub="회원 가입 → 구독 전환 → 파트너 전환 (실데이터)" action={<Pill tone="crypto"><UsersIcon className="size-3" /> 파트너 전환율 {mktRate}%</Pill>}>
          <div className="space-y-2.5">
            {funnel.map((f) => (
              <div key={f.name} className="flex flex-col gap-1.5 lg:flex-row lg:items-center lg:gap-4">
        ...
        <Panel title="초대한 회원" sub={`내 초대 코드로 가입한 직접 초대 ${total.toLocaleString()}명`}>
          <DataList
            columns={[
              { key: "uid", label: "회원", width: "1.4fr", mobile: "title", cell: (m) => <span className="font-semibold text-text-primary">{toUid(m.id)}</span> },
              { key: "role", label: "회원 구분", width: "1fr", mobile: "meta", cell: (m) => <Pill tone={ROLE_TONE[m.role]}>{ROLE_LABEL[m.role]}</Pill> },
              { key: "joined", label: "가입일", width: "1fr", mobile: "meta", cell: (m) => <span className="text-text-secondary tabular-nums">{toSeoulDate(m.created_at)}</span> },
              { key: "sub", label: "구독", align: "right", mobile: "value", cell: (m) => <Pill tone={m.is_active_subscriber ? "green" : "neutral"}>{m.is_active_subscriber ? "활성" : "미구독"}</Pill> },
            ]}
            rows={referred.slice(0, 20)}
            rowKey={(m) => m.id}
            empty="초대한 회원이 없습니다."
          />
          {referred.length > 20 ? (
