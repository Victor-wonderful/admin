...
import { GitBranchIcon, XIcon, Loader2Icon, CheckCircle2Icon, TriangleAlertIcon, LockIcon } from "lucide-react";
...
// 배치 대기 목록 + 배치 모달. 후원배치는 한 번만 — 확정 단계에서 다시 묻는다.
export function PlacementPanel({
  ...
  const [sel, setSel] = React.useState<PendingPlacement | null>(null);
  ...
  useEscapeKey(!!sel, close);

  const open = (p: PendingPlacement) => {
    setSel(p);
    setMode("recommended");
  ...
  const submit = () =>
    start(async () => {
      ...
      setDone(`${finalTarget?.uid ?? ""} 아래 ${res.slot}번 자리에 배치·확정되었습니다`);
      router.refresh();
    });
      ...
      <div className="rounded-lg bg-card p-5 ring-1 ring-warning/40 shadow-[0_2px_12px_-3px_rgba(16,24,40,0.08)]">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <div className="mt-0.5 text-xs text-text-secondary">내가 초대한 회원 중 구독을 시작했지만 아직 후원 조직에 자리가 없는 회원 · 자리는 한 번만 정할 수 있습니다</div>
          </div>
        </div>
        <div className="mt-3">
          ...
          {pending.map((p) => {
            ...
            return (
              <div key={p.id} className="grid grid-cols-[1.2fr_1fr_1fr_auto] items-center gap-3 border-b py-2.5 text-sm last:border-0">
                ...
                <button type="button" onClick={() => open(p)} className="inline-flex items-center gap-1 rounded-md bg-brand px-3 py-1.5 text-xs font-bold text-white">
        ...
        <div className="fixed inset-0 z-50 grid place-items-center p-4">
          ...
          <div className="relative z-10 w-full max-w-[520px] overflow-hidden rounded-xl bg-card shadow-[0_20px_40px_-8px_rgba(11,15,20,0.35)]">
            <div className="flex items-start justify-between border-b px-6 py-5">
              <div>
                <h2 className="text-base font-bold text-text-primary">후원배치 · {sel.uid}</h2>
            ...
            {done ? (
              ...
              <>
                <div className="space-y-4 px-6 py-5">
                  ...
                  <label className={cn("flex cursor-pointer items-start gap-3 rounded-lg p-3.5 ring-1 transition-colors", mode === "custom" ? "bg-green-50 ring-green-500" : "ring-border")}>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-1 text-[11px] text-text-tertiary">들여쓰기 = 깊이 · 선택한 회원 아래 다음 빈 자리(2번 이상)에 들어갑니다</div>
                    </div>
                  </label>
                ...
                <div className="flex items-center justify-between gap-2 border-t px-6 py-4">
                  <span className="text-[12px] text-text-secondary">배치 위치: <b className="text-text-primary">{finalTarget?.uid ?? "—"}</b> 아래</span>
