...
// MarketerDashboardView 본문 — 페이지(세션 가드)와 분리해 두면 재사용·검증이 쉽다.
export async function MarketerDashboardView({ viewerId }: { viewerId: string }) {
  const ME = viewerId;  const [rank, wd, settle, cumulative, code, referred, me, annual, subs, pendingPlacements] = await Promise.all([
    getMemberRank(ME),
    ...
    <>
      ...
      <div className="flex-1 space-y-4 overflow-auto bg-canvas p-4 lg:p-7">
        ...
        {pendingPlacements.length > 0 ? (
          <Link href="/marketer/genealogy" className="flex flex-col gap-2 rounded-xl bg-warning-soft px-4 py-3.5 ring-1 ring-warning/30 transition-colors hover:ring-warning lg:flex-row lg:items-center lg:justify-between lg:gap-4 lg:px-5">
        ...
        <div className="flex flex-col gap-4 rounded-[20px] bg-feature p-5 text-white shadow-[0_2px_12px_-3px_rgba(16,24,40,0.12)] lg:flex-row lg:items-center lg:justify-between">
          ...
          {activeSub ? (
            <a
              ...
              className="inline-flex h-12 shrink-0 items-center justify-center gap-2 rounded-[10px] bg-white px-5 text-sm font-bold whitespace-nowrap text-green-700 lg:h-auto lg:py-2.5"
            >
              <ExternalLinkIcon className="size-4" /> 포르투나 앱 열기
            </a>
          ) : (
            <Link href="/marketer/orders" className="inline-flex h-12 shrink-0 items-center justify-center gap-2 rounded-[10px] bg-white px-5 text-sm font-bold whitespace-nowrap text-green-700 lg:h-auto lg:py-2.5">
              구독 갱신
            </Link>
          )}
        </div>
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_392px]">
          ...
          <div className="flex flex-col justify-between rounded-[20px] bg-gradient-to-br from-lime to-green-600 p-[22px] text-white shadow-[0_2px_12px_-3px_rgba(16,24,40,0.12)]">
            ...
            <div className="my-3 text-[32px] leading-none font-bold tabular-nums lg:my-0 lg:text-[38px]">{usd(balance)} <span className="text-base font-semibold text-white/80">USDT</span></div>
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          {kpis.map((k) => (
            <div key={k.label} className={cn("flex items-center gap-2.5 rounded-[14px] bg-card p-3 ring-1 ring-border shadow-[0_2px_12px_-3px_rgba(16,24,40,0.08)] lg:gap-3 lg:p-4")}>
              <span className={cn("grid size-9 shrink-0 place-items-center rounded-[12px] lg:size-10", k.badge)}><k.icon className="size-[17px] lg:size-[19px]" /></span>
              <div className="min-w-0">
                <div className="text-[11px] text-text-secondary lg:text-xs">{k.label}</div>
                <div className="text-lg font-bold tabular-nums text-text-primary lg:text-xl">{k.value}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        {/* RowC — 리워드 구성 도넛 + 내 초대 */}
        <div className="grid gap-4 lg:grid-cols-[1fr_392px]">
          <div className={cn(CARD, "flex flex-col items-center gap-5 sm:flex-row sm:gap-7")}>
        ...
        <div className={cn(CARD, "pt-3")}>
          ...
          <DataList
            columns={[
              { key: "cycle", label: "사이클", mobile: "meta", cell: () => <span className="text-text-tertiary tabular-nums">{CYCLE}</span> },
              { key: "type", label: "유형", width: "1fr", mobile: "title", cell: (c) => <Pill tone={c.dot.includes("green") ? "green" : c.dot.includes("crypto") ? "crypto" : "info"}>{c.label.replace(" 리워드", "")}</Pill> },
              { key: "desc", label: "내역", width: "1.5fr", mobile: "meta", cell: () => <span className="text-text-secondary">당월 정산 적립</span> },
              { key: "amount", label: "금액", align: "right", mobile: "value", cell: (c) => <span className="font-bold tabular-nums text-green-700">+{usd(c.v)}</span> },
            ]}
            rows={comp.filter((c) => c.v > 0)}
            rowKey={(c) => c.label}
            empty="당월 리워드 내역이 없습니다."
          />
        </div>
      </div>
    </>
