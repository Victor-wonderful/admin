...
// 등록회원 홈 본문 — 페이지(세션 가드)와 분리해 두면 재사용·검증이 쉽다.
export async function RegisteredHome({ member }: { member: MemberRow }) {
  const ME = member.id;
      ...
      <div className="flex-1 space-y-4 overflow-auto p-4 lg:p-7">
        ...
        <div className="flex flex-col gap-4 rounded-xl bg-gradient-to-br from-lime to-green-600 p-5 text-white shadow-[0_2px_12px_-3px_rgba(16,24,40,0.12)] lg:flex-row lg:items-center lg:justify-between lg:p-6">
          <div className="min-w-0 space-y-2">
            ...
            <h2 className="text-[20px] font-bold lg:text-[23px]">포르투나 플랫폼을 체험해 보세요</h2>
          ...
          <div className="flex w-full shrink-0 flex-col gap-2 lg:w-auto lg:items-end">
            ...
            <span className="text-xs font-medium text-white/80 lg:text-right">Fortuna 앱 · 새 탭에서 열림</span>
          </div>
        </div>
        ...
        >
          {/* 폰: 세로 목록(라벨이 잘리지 않음) / lg 이상: 기존 가로 스테퍼 */}
          <div className="flex flex-col gap-3 lg:hidden">
            {steps.map((s) => (
              <div key={s.label} className="flex items-center gap-3">
                ...
                {s.state === "done" ? <CheckIcon className="size-4 shrink-0 text-brand" /> : null}
              </div>
            ))}
          </div>
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_392px]">
          <Panel>
            <div className="flex items-center gap-3">
              <div className="grid size-11 shrink-0 place-items-center rounded-[13px] bg-green-50 text-green-700">
          ...
          <Panel title="지갑 입금" sub="구독 결제를 위해 회사 입금 주소로 USDT 를 보내세요">
            <div className="space-y-3.5">
              <div className="flex items-center justify-between gap-3 rounded-lg bg-surface-muted p-3.5 ring-1 ring-border">
                ...
                <span className="grid size-9 shrink-0 place-items-center rounded-[10px] bg-green-50 text-green-700"><WalletIcon className="size-[18px]" /></span>
              </div>
