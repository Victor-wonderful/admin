...
type Item = { id: string; tone: "info" | "warning" | "negative" | "green"; title: string; sub: string; href?: string };
...
export function NotificationBell() {
  ...
  const endpoint = isAdmin ? "/api/admin/notifications" : "/api/me/notifications";
          ...
          <div className="max-h-[360px] overflow-auto">
            {loading && items === null ? (
              ...
              items.map((it) => {
                ...
                return it.href ? (
                  <Link key={it.id} href={it.href} onClick={() => setOpen(false)} className="flex items-start gap-3 border-b px-4 py-3 transition-colors last:border-0 hover:bg-n-50">
                    {body}
                  </Link>
                ) : (
                  <div key={it.id} className="flex items-start gap-3 border-b px-4 py-3 last:border-0">{body}</div>
                );
              })
            )}
          </div>
