...
export async function getMembersView(role?: MemberRole) {
  ...
  const all: ExplorerRow[] = members.map((m) => {
    ...
    return {
      ...
      recommenderName: m.recommender_id && known.has(m.recommender_id) ? toUid(m.recommender_id) : null,
      joinedAt: m.created_at.slice(0, 10),
      // 파트너만 직급 보유. rank=0(무직급)도 파트너면 명시, 비파트너는 null.
